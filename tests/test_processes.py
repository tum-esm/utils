...
def test_ungraceful_process() -> None:
    ...
    expected_pid = start_background_process(sys.executable, SCRIPT_PATH, waiting_period=2)
    time.sleep(0.1)
...
def test_graceful_process() -> None:
    ...
    assert get_process_pids(SCRIPT_PATH_GRACE) == [expected_pid]
    time.sleep(0.1)

    # cannot terminate because teardown takes long
    assert terminate_process(SCRIPT_PATH_GRACE) == [expected_pid]
    time.sleep(0.5)
    assert get_process_pids(SCRIPT_PATH_GRACE) == [expected_pid]

    # hard kill works
    assert terminate_process(SCRIPT_PATH_GRACE, termination_timeout=0) == [expected_pid]
    time.sleep(0.1)
    assert get_process_pids(SCRIPT_PATH_GRACE) == []
